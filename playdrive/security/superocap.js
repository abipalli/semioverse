...
function CardFactory() {
  class Card extends Map {
    constructor(cardFactory, name, ruleEngine = async () => true, ...args) {
      super(...args);
      this.name = name;
    ...
    */
        ...
        if (
          ...
          if (Array.isArray(path)) {
            ...
            if (!map.has(actualPath)) {
              map.set(actualPath, this.cardFactory.createCard());
            }
            map = map.get(actualPath);
          } else {
            if (!map.has(path)) {
              console.log("foseihfoseifh:", this.cardFactory.createCard());
              map.set(path, this.cardFactory.createCard());
            }
            map = map.get(path);
          }
        } else {
      ...
      for await (const route of routes) {
        ...
        while (!result.done) {
          ...
          if (nextResult.done && result.value.has(destinationkey)) {
            result.value.set(destinationkey, this.cardFactory.createCard());
    ...
    async picture(depth) {
      ...
      const deepCopyAndFreeze = (source, depth = 0) => {
        const destination = this.cardFactory.createCard();
          ...
          if (d < depth) {
            for (const [key, value] of src.entries()) {
              if (value instanceof Card || value instanceof CardProxy) {
                const childCopy = this.cardFactory.createCard();
  ...
  return {
    createCard: (name, ruleEngine = async () => true, ...args) => {
      let card = new Card(CardFactory(), name, ruleEngine, ...args);
...
function runnerExtension(card) {
  ...
  card.set("state", this.cardFactory.createCard());
  card.set("delegators", this.cardFactory.createCard());
